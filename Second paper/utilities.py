...
def plot_feature_importances_regression(X, y):
    ...
    for name, model in models.items():
        ...
        # Handle feature importances (for tree-based models)
        if hasattr(model, 'feature_importances_'):
            importances[name] = dict(sorted(zip(X.columns if hasattr(X, 'columns') else range(X.shape[1]), 
                                                model.feature_importances_), key=lambda x: x[1]))
        # Handle coefficients (for linear models)
        elif hasattr(model, 'coef_'):
            importances[name] = dict(sorted(zip(X.columns if hasattr(X, 'columns') else range(X.shape[1]), 
                                                np.abs(model.coef_)), key=lambda x: x[1]))
